import React, { ChangeEvent, KeyboardEvent, useReducer, useRef, useState } from 'react';
import './style.css';
...
export default function Authentication() {
  ...
  //    state: 화면 상태    //
  const [view, setView] = useState<'sign-in'|'sign-up'>('sign-in');//리터럴상태
  //    state: 쿠키 상태    // 
  ...
  const SignUpCard = () => {
    ...
    //    event handler: 상세 주소 변경 변경 이벤트 처리    //
    ...
    //    event handler: 패스워드 버튼 클릭 이벤트 처리    //
    const onPasswordButtonClickhnadler = () => {
    ...
    const onPasswordCheckButtonClickhnadler = () => {
    ...
    return(
      <div className='auth-card'>
        <div className='auth-card-box'>
          <div className='auth-card-top'>
            ...
            {page === 1 && (
              <>
                ...
                <InputBox ref={passwordRef} label='비밀번호*' type={passwordType} placeholder='비밀번호를 입력해주세요.' value={password} onChange={onPasswordChangeHandler} error={isPasswordError} message={passwordErrorMessage}   icon={passwordButtonIcon} onButtonClick={onPasswordButtonClickhnadler} onKeyDown={onPasswordKeyDownHandler}/>
                <InputBox ref={passwordCheckRef} label='비밀번호 확인*' type={passwordCheckType} placeholder='비밀번호를 다시 입력해주세요.' value={passwordCheck} onChange={onPasswordCheckChangeHandler} error={isPasswordCheckError} message={passwordCheckErrorMessage} icon={passwordCheckButtonIcon} onButtonClick={onPasswordCheckButtonClickhnadler} onKeyDown={onPasswordCheckKeyDownHandler}/>
              </>
            )}
            {page === 2 && (
              <>
          ...
          <div className='auth-card-bottom'>
            {page === 1 && (
              <div className='back-large-full-button' onClick={onNextButtonClickHandler}>{'다음단계'}</div>
            )}
            {page === 2 && (
              <>
                ...
                <div className='back-large-full-button' onClick={onSignUpButtonClickHandler}>{'회원가입'}</div>
              </>
            )}
            <div className='auth-description-box'>
